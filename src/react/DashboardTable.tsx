      ...
      <section className="overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-sm">
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-xs text-slate-500">
                Filtre por status ou busque por palavras-chave.
              </p>
            </div>
          </div>
          <div className="flex items-center gap-3">
        ...
        <div className="p-6">
          <div className="mb-4 flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
            <div className="inline-flex w-fit flex-wrap gap-1.5 rounded-lg border border-slate-200 bg-white p-1 shadow-sm">
              {STATUS_TABS.map((t) => {
            ...
            >
              ...
              {authExpired ? (
                <a
                  ...
                  className="inline-flex items-center gap-1.5 rounded-md bg-red-600 px-4 py-2 text-xs font-semibold text-white shadow-sm transition hover:bg-red-700"
                >
                  Entrar novamente
                </a>
              ) : (
          ...
          {hydrated && filtered.length === 0 && !error && (
            <div className="rounded-lg border border-dashed border-slate-300 bg-slate-50/40 py-12 text-center">
              <span className="mx-auto inline-grid h-14 w-14 place-items-center rounded-full bg-white text-slate-500 shadow-sm ring-1 ring-slate-200">
                {documents.length === 0 ? (
                  <FilePlus2 size={26} />
                ) : (
                  <Inbox size={26} />
              ...
              <p className="mx-auto mt-1 max-w-sm text-sm text-slate-500">
                {documents.length === 0
                  ? 'Clique em “Novo Documento” acima para criar seu primeiro levantamento.'
          ...
          {filtered.length > 0 && (
            <div className="overflow-x-auto rounded-lg border border-slate-200 bg-white">
              <table className="w-full min-w-160 divide-y divide-slate-200 text-sm [&_th]:max-w-max [&_td]:max-w-max [&_th]:whitespace-nowrap [&_td]:whitespace-nowrap">
